...
{: .info }
> If a document has been [fetched]({% link manipulating-a-document/fetching-and-subscribing.md %}), and its `type` remains unset, then the document has not yet been created.
...
{: .warn }
> The data will only be available after a document has been [fetched or subscribed to]({% link manipulating-a-document/fetching-and-subscribing.md %}).
...
Apply an operation to the document locally and send the operation to the server. The document must have been [fetched or subscribed to]({% link manipulating-a-document/fetching-and-subscribing.md %})
...
{: .info }
ShareDB documents and their ops are never truly deleted from the database. Instead, they will be [tombstoned]({% link manipulating-a-document/deleting-a-document.md %}).
